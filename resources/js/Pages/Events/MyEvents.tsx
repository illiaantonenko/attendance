...
function EventCard({ event, showStatus = false }: { event: EventItem; showStatus?: boolean }) {
    ...
    return (
        <div className="bg-white rounded-xl border border-gray-200 p-4 hover:border-blue-300 transition-colors shadow-sm">
            <div className="flex justify-between items-start mb-3">
                <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                        <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${eventTypeColors[event.event_type] || 'bg-gray-100 text-gray-800'}`}>
                            {eventTypeLabels[event.event_type] || event.event_type}
                        </span>
                        {showStatus && (
                            <span className={`px-2 py-0.5 text-xs font-medium rounded-full ${event.is_checked_in ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <AuthenticatedLayout title="Мої заняття">
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
                        <p className="text-sm text-gray-500">Відвідано</p>
                ...
                {upcoming.length > 0 && (
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900 mb-4">Найближчі заняття</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {past.length > 0 && (
                    <div>
                        <h2 className="text-lg font-semibold text-gray-900 mb-4">Історія</h2>
                        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Дата</th>
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Заняття</th>
                ...
                {upcoming.length === 0 && today.length === 0 && past.length === 0 && (
                    <div className="bg-white rounded-xl border border-gray-200 p-12 text-center shadow-sm">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 mb-2">Немає занять</h3>
                        <p className="text-gray-500">Ви ще не належите до жодної групи або для вашої групи ще не створено занять.</p>
                    </div>
                )}
            </div>
        </AuthenticatedLayout>
